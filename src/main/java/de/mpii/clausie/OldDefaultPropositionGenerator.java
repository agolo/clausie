 ...
 */
public class OldDefaultPropositionGenerator extends OldPropositionGenerator {

    public OldDefaultPropositionGenerator(ClausIE clausIE) {
        super(clausIE);
    }

	@Override
	public void generate(List<OldProposition> result, Clause clause,
			List<Boolean> include) {
		OldProposition proposition = new OldProposition();
		List<OldProposition> propositions = new ArrayList<>();
		...
		for(Integer index: sortedIndexes) {
			if (clause.constituents.get(clause.verb) instanceof IndexedConstituent && clause.adverbials.contains(index) && ((IndexedConstituent)clause.constituents.get(index)).getRoot().index() < ((IndexedConstituent)clause.constituents.get(clause.verb)).getRoot().index()) continue;
				for(OldProposition p: propositions) {
