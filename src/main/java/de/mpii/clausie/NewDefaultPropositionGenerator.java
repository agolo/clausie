 ...
 */
public class NewDefaultPropositionGenerator extends NewPropositionGenerator {

    public NewDefaultPropositionGenerator(ClausIE clausIE) {
        super(clausIE);
    }

    @Override
    public void generate(List<NewProposition> result,
                         Clause clause,
                         List<Boolean> include) {
        NewProposition proposition = new NewProposition();
        List<NewProposition> propositions = new ArrayList<>();
        ...
        for (Entry<String, SortedSet<Integer>> entry : sortedIndexes.entrySet()) {
            for (Integer index : entry.getValue()) {
                if (clause.constituents.get(clause.verb) instanceof IndexedConstituent && clause.adverbials.contains(index) && ((IndexedConstituent) clause.constituents.get(index)).getRoot().index() < ((IndexedConstituent) clause.constituents.get(clause.verb)).getRoot().index())
                    continue;
                for (NewProposition p : propositions) {
